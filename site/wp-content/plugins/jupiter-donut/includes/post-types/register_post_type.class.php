...
class Mk_Register_Post_Type {
	...
	function register_post_type() {
		...
		$labels = array_merge(
			array(
				'name' => sprintf( _x( '%s', 'post type general name', 'jupiter-donut' ) , $this->plural ),
				'singular_name' => sprintf( _x( '%s', 'post type singular title', 'jupiter-donut' ) , $this->title ),
				'menu_name' => sprintf( __( '%s', 'jupiter-donut' ) , $this->plural ),
				'all_items' => sprintf( __( 'All %s', 'jupiter-donut' ) , $this->plural ),
				'add_new' => sprintf( _x( 'Add New', '%s', 'jupiter-donut' ) , $this->title ),
				'add_new_item' => sprintf( __( 'Add New %s', 'jupiter-donut' ) , $this->title ),
				'edit_item' => sprintf( __( 'Edit %s', 'jupiter-donut' ) , $this->title ),
				'new_item' => sprintf( __( 'New %s', 'jupiter-donut' ) , $this->title ),
				'view_item' => sprintf( __( 'View %s', 'jupiter-donut' ) , $this->title ),
				'items_archive' => sprintf( __( '%s Archive', 'jupiter-donut' ) , $this->title ),
				'search_items' => sprintf( __( 'Search %s', 'jupiter-donut' ) , $this->plural ),
				'not_found' => sprintf( __( 'No %s found', 'jupiter-donut' ) , $this->plural ),
				'not_found_in_trash' => sprintf( __( 'No %s found in trash', 'jupiter-donut' ) , $this->plural ),
				'parent_item_colon' => sprintf( __( '%s Parent', 'jupiter-donut' ) , $this->title ),
			) , $this->labels
		);
